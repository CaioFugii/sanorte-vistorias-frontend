...
import { AceitePhoto, collectAceitePhotos, findFieldValue, getImageFormat, loadImageAsDataUrl } from "../helpers";
...
function drawTitleAndStretch(
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9.2);
  doc.text("ACEITE DE RECOMPOSICAO DE PAVIMENTO", MARGIN + contentWidth / 2, y + 5.4, { align: "center" });
...
function drawAceiteSignatureFooter(doc: jsPDF, startY: number): number {
  const pageWidth = doc.internal.pageSize.getWidth();
  const blockWidth = (pageWidth - MARGIN * 2) / 4;
  const titles = ["PREFEITURA", "FISCALIZACAO", "DE ACORDO - EMPREITEIRA", "SABESP"];
  const names = [
...
export async function generateAceitePavimentoPdf(input: ReportPdfInput): Promise<void> {
  ...
  const photos = collectAceitePhotos(orderedFields, formData);
  ...
  cursorY = drawTitleAndStretch(doc, cursorY + 1, bacia, logradouro, montante, jusante) + 1.5;
  ...
  drawAceiteSignatureFooter(doc, cursorY + 1.5);
  ...
  drawAceiteSignatureFooter(doc, cursorY + 8);
